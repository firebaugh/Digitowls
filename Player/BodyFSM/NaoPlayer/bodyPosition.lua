...
function entry()
  print("Player: " .. Config.game.robotID .. " state: " .. _NAME .. " entry");
...
function update()
  ...
  ball = wcm.get_ball();
  pose = wcm.get_pose();
  ballCurrGlobal = util.pose_global({ball.x, ball.y, 0}, {pose.x, pose.y, pose.a});
  tBall = Body.get_time() - ball.t;

  role = gcm.get_team_role();
  if (role == 2) then
    -- defend
    homePosition = .6 * ballCurrGlobal;
  ...
  elseif (role == 3) then
    ...
    homePosition[2] = -1*util.sign(ballCurrGlobal[2]) * .75;

    -- face ball 
    homePosition[3] = ballCurrGlobal[3];
  else
    -- attack
    homePosition = ballCurrGlobal;
  end
