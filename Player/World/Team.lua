...
role = -1;
neighbor = wcm.get_pose(); --init nearest neighbor pose = robot's pose
count = 0;
...
function update()
  ...
  for id = 1,4 do --for each player on team

    if not states[id] then
      -- no message from player have been received
      eta[id] = math.huge;
      ddefend[id] = math.huge;
      player_pos = {math.huge,math.huge,math.huge};

    else
      -- player's pose
      player_pose[id] = states[id].pose();
  ...
  state.neighbor = set_neighbor(player_pos,state.id);
...
function update_shm() 
  ...
  gcm.set_team_neighbor(neighbor);
end
...
--calculate each players nearest neighbor
--store that position in gcm.lua team.neighbor
function set_neighbor(player_pos, curr_player_id)
  curr_player_pos = player_pos[curr_player_id]

  for id = 1,4 do
    if id == curr_player_id then
